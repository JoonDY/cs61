...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        if place.ant is None:
            ...
            place.ant += [self]
            print('DEBUG: ', place.ant) 
        elif place.ant and isinstance(place.ant, ContainerAnt):
            place.ant += [self]
            print('DEBUG: ', place.ant) 
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_range, current_place, next_place = 0, self.place, self.place.entrance
        while current_place != beehive:
            if current_place.bees and self.min_range <= current_range <= self.max_range:
                return rANTdom_else_none(current_place.bees)
            current_place, next_place = next_place, next_place.entrance
            current_range += 1
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, gamestate):
        ...
        current_place, before_place = self.place, self.place.exit
        while current_place:
            if before_place and before_place.ant and not before_place.ant.buffed and before_place.ant.damage:
                before_place.ant.damage *= 2
                before_place.ant.buffed = True
            if not before_place:
                break
            current_place, before_place = before_place , before_place.exit
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.original: 
            bees_win()
